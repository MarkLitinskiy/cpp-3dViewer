...
void MyGLWidget::SetCoordinates() {
    dataCoordinates.num_x = ui->double_spin_box_translate_ox->value();
...
void MyGLWidget::move() {
  if (v && f){
    dataCoordinates.state = affine::kTranslate;
    controller->SubMethods(dataCoordinates);
  }
  update();
}
...
void MyGLWidget::rotate(affine concord) {
    if (v && f) {
        dataCoordinates.state = concord;
        controller->Rotation(dataCoordinates);
    }
      update();
}
...
void MyGLWidget::scale() {
  if (v && f){
      dataCoordinates.state = affine::kScale;
      controller->SubMethods(dataCoordinates);
  }
  update();
}
...
void MyGLWidget::centerFigure() {
  if (v && f) {
      dataCoordinates.state = affine::kCenter;
      controller->SubMethods(dataCoordinates);
    reduction();
  }
  update();
...
void MyGLWidget::reduction() {
    if (v && f){
        dataCoordinates.state = affine::kObjectReduct;
        controller->SubMethods(dataCoordinates);
    }
}
